...
type RawStockUnitDetail = Omit<StockUnitDetail, 'expirationDate' | 'createdAt' | 'openedAt' | 'consumedAt'> & {
...
export class ApiStockUnitService implements IStockUnitService {
    ...
    async getByLocation(locationId: string): Promise<StockUnitDetail[]> {
        const raw = await apiClient.get<RawStockUnitDetail[]>(`/api/stock-units/location/${locationId}`)
        return raw.map(parseStockUnit)
    }
